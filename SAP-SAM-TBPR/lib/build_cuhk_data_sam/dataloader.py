...
import torch
from .dataset import TBPRBaseDataset, CUHKPEDE_img_dateset, CUHKPEDE_txt_dateset


def get_dataloader(opt,sam_processor):
    """
    tranforms the image, downloads the image with the id by data.DataLoader
    """

    if opt.mode == 'train':
        dataset = TBPRBaseDataset(opt,sam_processor)
        ...
        img_dataset = CUHKPEDE_img_dateset(opt,sam_processor)
