...
class LRSchedulerWithWarmup(_LRScheduler):
    def __init__(
        ...
        self.rng_adj = rng_adj
        super().__init__(optimizer, last_epoch)

    def get_lr(self):
        # print(self.last_epoch)
        # print(len(self.base_lrs))
        # print(self.base_lrs)
        ...
        if self.last_epoch < self.total_epochs:
            epoch_temp = self.last_epoch
        else:
            epoch_temp = self.total_epochs+self.rng_adj*sin((self.last_epoch-self.total_epochs)*pi/self.rng_step*0.5)
        epoch_ratio = (epoch_temp - self.warmup_epochs) / (
