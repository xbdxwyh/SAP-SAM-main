...
class CUHKPEDES(BaseDataset):
    ...
    def __init__(self, root='', verbose=True, part_seg=False):
        ...
        if part_seg:
            ...
            with open(op.join(self.dataset_dir, "CUHK-PEDES_data_final_68126.json"),"r+") as f:
                data = json.load(f)
            
            # 读取使用SAM分割后的所有数据集名称以及置信度
            with open(op.join(self.dataset_dir, "CUHK-PEDES_score_dict_68125.json"),"r+") as f:
            ...
            for data_id,item in enumerate(data):
                ...
                # 分割结果的存放
                seg_img_name = ["_".join([str(data_id)] + item['name'][:-len(".png")].split("/")+[str(i)]) for i in range(len(item['attribute']))]
