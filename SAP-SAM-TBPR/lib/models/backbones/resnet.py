...
class ResNet(nn.Module):
    def __init__(
        ...
        self.pool = nn.AdaptiveAvgPool2d((1, 1)) if avg else nn.AdaptiveMaxPool2d((1, 1))
    ...
    def forward(self, x):
        ...
        #feature_x_layer_1 = self.pool(x_layer1).squeeze(dim=-1).squeeze(dim=-1)

        x_layer2 = self.layer2(x_layer1)
        #feature_x_layer_2 = self.pool(x_layer2).squeeze(dim=-1).squeeze(dim=-1)

        x_layer3 = self.layer3(x_layer2)
        feature_x_layer_3 = self.pool(x_layer3).squeeze(dim=-1).squeeze(dim=-1)

        x_layer4 = self.layer4(x_layer3)
        g_feature_x_layer_4 = self.pool(x_layer4).squeeze(dim=-1).squeeze(dim=-1)
        l_feature_x_layer_4 = [self.pool(x_layer4[:, :, i*4:(i+1)*4, :]).squeeze(dim=-1).squeeze(dim=-1) for i in range(6)]
        #x_layer5 = self.avgpool(x)

        return x_layer1,x_layer2,feature_x_layer_3,g_feature_x_layer_4,l_feature_x_layer_4
