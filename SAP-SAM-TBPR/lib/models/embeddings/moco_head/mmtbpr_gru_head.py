...
class MoCoHead(nn.Module):
    def __init__(
        ...
        self.pooler = cfg.MODEL.VISUAL_POOLER
        ...
        if self.fc:
            self.v_fc_q = nn.Sequential(
                nn.Linear(cfg.HIDDEN_SIZE, self.embed_size),
                nn.ReLU(),
        ...
        self.v_embed_layer = nn.Linear(cfg.HIDDEN_SIZE, self.embed_size)
    ...
    def forward(self, images, captions):
        ...
        if self.pooler:
            v_embed = self.v_encoder_q(pixel_values = images).pooler_output
        else:
            v_embed = self.v_encoder_q(pixel_values = images).last_hidden_state[:,0]
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                if self.pooler:
                    v_embed_k = self.v_encoder_k(pixel_values = images).pooler_output
                else:
                    v_embed_k = self.v_encoder_k(pixel_values = images).last_hidden_state[:,0]
                if self.fc:
            ...
            #return v_embed, t_embed, v_pos, v_neg, t_pos, t_neg, id_q
