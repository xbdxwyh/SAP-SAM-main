...
class CLIP(nn.Module):
    ...
    def make_image_mask(self, images, image_labels, mask_prob, part_mask_prob, mask_token):
        x = self.visual.conv1(images.type(self.dtype))  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        batchsize = x.shape[0]
        seq_len = x.shape[1]
        ...
        part_mask = torch.cat([~cls_labels.to(x.device),(torch.randn((batchsize,seq_len))<part_mask_prob).to(x.device) * image_labels.bool()],dim=1) 
        bck_mask = torch.cat([~cls_labels.to(x.device),(torch.randn((batchsize,seq_len))<mask_prob).to(x.device) * ~image_labels.bool()],dim=1)
        img_token_mask = (part_mask+bck_mask).reshape(-1)
        img_token_mask_id = torch.nonzero(img_token_mask)
        img_feature_gt = x.reshape(-1,feature_dim)[img_token_mask_id.reshape(-1)]

        image_feats_masked = x.reshape(-1,feature_dim)*~img_token_mask.unsqueeze(-1) + mask_token.expand(batchsize*(seq_len+1),-1)*img_token_mask.unsqueeze(-1)
        image_feats_masked = image_feats_masked.reshape(batchsize,(seq_len+1),-1)
        x = image_feats_masked
        x = x.to(self.dtype)
        ...
        return x,img_feature_gt,img_token_mask_id
