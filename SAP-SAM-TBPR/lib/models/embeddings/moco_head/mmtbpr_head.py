...
from ...backbones.bert_gru import BERTwithGRUModel
...
class MoCoHead(nn.Module):
    ...
    @torch.no_grad()
    def get_text_embedding(self,captions,length,mask):
        t_embed = self.t_encoder_q.forward_mmtbpr(
            captions = captions,
            text_length = length,
            captions_mask = mask,
        )
    ...
    def forward(self, images, captions,caption_length, mask, label):
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                t_embed_k = self.t_encoder_k.forward_mmtbpr(
                    captions = captions,
                    text_length = caption_length,
                    captions_mask = mask
                )
            ...
            losses = self.loss_evaluator(
                v_embed, t_embed,None,None,None,None, v_pos, v_neg, t_pos, t_neg, id_q
            )
